...
namespace marian {
  ...
  void ExpressionGraphONNXExporter::exportToONNX(const std::string& modelToPrefix, Ptr<Options> modelOptions, const std::vector<std::string>& vocabPaths, const bool shortlistFlag)
  {
    ...
    const size_t batchSize = 1; // @BUGBUG change to some other primary number if you want to try a batch size other than 1  
    const size_t shortlistDim = sentinelDim * 50;
	...
	auto dataInput = extractInputByName(inputName);
        embeddingInputs.push_back(dataInput);
        if (forEncoder) {
    ...
    for (size_t batchIndex = 0; batchIndex < numEncoders; batchIndex++) {
      auto sb = New<data::SubBatch>(batchSize, sentinelDim, vocabs[batchIndex]);
    ...
    if (shortlistFlag) {
      // Generate a random shortlist with a distinct shape
      std::unordered_set<WordIndex> shortlistSet;
      for (int i = 0; i < shortlistDim; i++) {
        shortlistSet.emplace(i + 5); // ignore top 5 words in case it's unk or sth
      }

      auto shortlist = New<data::FakeShortlistGenerator>(shortlistSet);
      model->setShortlistGenerator(shortlist);
    }
    ...
    if (shortlistFlag)
      inputs.emplace_back(decodeShortlistInput);
    ...
    for (const auto& dss : extractStates(decodeFirstState)) {
      inputs.emplace_back(std::make_pair("decoder_state_" + std::to_string(inputs.size() - (numEncoders*2 + 2) - 1), dss));
    }
    ...
    serializeToONNX(modelToPrefix, std::move(functionDefs), sentinelDim, shortlistDim);
  }
}
